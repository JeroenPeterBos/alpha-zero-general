...
class Coach():
    ...
    def __init__(self, game, nnet, args, opponents=None):
        self.game = game
        self.nnet = nnet
    ...
    def learn(self):
        ...
        for i in range(1, self.args.numIters + 1):
            ...
            def previous_model(x):
                return np.argmax(pmcts.getActionProb(x, temp=0))

            def new_model(x):
                return np.argmax(nmcts.getActionProb(x, temp=0))

            arena = Arena(previous_model, new_model, self.game, render=self.args.render)
    ...
    def pitAgainstOpponents(self):
        ...
        for player in self.opponents:
            ...
            fwins, swins, draws = arena.playGames(self.args.arenaCompare // len(self.opponents))

            print('MODEL/PLAYER WINS : %d / %d ; DRAWS : %d' % (swins, fwins, draws))
